...
const Navbar = () => {
  ...
  return (
    <nav className="sticky w-full top-0 z-50 border-b">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative dropdown-container">
              {currentUser ? (
                ...
                <Link to="/signin" className="px-4 py-2 rounded-md">
                  Login
                </Link>
              )}
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-48 border rounded-lg shadow bg-gray-600">
                  <Link
                    ...
                    className="block px-4 py-3 hover:opacity-70"
                  >
                    Profile
                  </Link>
                  <button
                    onClick={handleClick}
                    className="w-full text-left px-4 py-3 hover:opacity-70"
                  >
                    Logout
                  </button>
                </div>
      ...
      <div
        className={`fixed left-0 top-0 h-full w-64 shadow z-50 transform ${
          isDrawerOpen ? "translate-x-0" : "-translate-x-full"
        } transition-transform duration-300 ease-in-out`}
      >
        <div className="p-4">
          <button
            className="text-2xl absolute right-1 top-5"
            onClick={closeAll}
          >
            <MdClose />
          </button>
          ...
          <div className="mt-8 space-y-4">
            {currentUser && (
              <Link
                to="/profile"
                onClick={closeAll}
                className="block p-2 hover:opacity-70"
              >
                Profile
              </Link>
            )}
